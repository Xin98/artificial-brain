...
| Clean-context regression | `make verify && make migration-test && make smoke-test` re-run independently plus `git diff --check`, acceptance mapping, zone compliance, credential scan; immutable reviewer reports are `regression-report.md` and, after the D9 fix, the superseding `regression-report-v2.md` | `2049071`; superseded by `fa139f6` | Complete (PASS ×2) |
