-- River v0.44.0 schema, inlined from rivermigrate migrations; upgrade by appending a new migration
--
-- Source: github.com/riverqueue/river/riverdriver/riverpgxv5 v0.44.0,
-- migration/main/002_initial_schema.up.sql through
-- 007_notification_outbox_sqlite_jsonb_and_sql_cleanup.up.sql, concatenated in
-- numeric order. The "TEMPLATE: schema" markers were removed; River substitutes
-- an empty prefix for them when targeting the default public schema.
--
-- River's migration 001_create_river_migration was intentionally NOT inlined: it
-- only creates River's own river_migration version tracking table. Tern owns
-- schema versioning here (public.schema_version), and River's migration 005 is
-- explicitly written to tolerate that table's absence. Do not create it.

-- rivermigrate main/002_initial_schema.up.sql
...
-- rivermigrate main/003_river_job_tags_non_null.up.sql
...
-- rivermigrate main/004_pending_and_more.up.sql
...
-- rivermigrate main/005_migration_unique_client.up.sql
...
-- rivermigrate main/006_bulk_unique.up.sql
...
-- rivermigrate main/007_notification_outbox_sqlite_jsonb_and_sql_cleanup.up.sql
...
-- Reverse of the up section: every object still created above, dropped in
-- reverse dependency order. Objects the up section creates and later drops
-- itself (trigger river_notify, function river_job_notify, index
-- river_job_kind_unique_key_idx, tables river_client and river_client_queue)
-- are guarded with IF EXISTS so the drop section stays a faithful inverse.
