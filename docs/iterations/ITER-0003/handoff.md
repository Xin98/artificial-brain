...
Tasks 1–16 are implemented and verified, and the independent clean-context regression returned PASS. The implementation HEAD is `2ee849e` (`build: wire ITER-0003 into migration and smoke gates`); the ledger-finalization commit `8ef71d6` (`ci: finalize ITER-0003 verification evidence`) and the regression record `2049071` (`docs: record ITER-0003 regression approval`) sit on top of it.
